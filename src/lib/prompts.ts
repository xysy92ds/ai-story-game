...
export function buildResolveUserMessage(room: RoomRow, history: StorySegment[], actions: RoundAction[]): string {
  ...
  const worldExtra = room.custom_world?.trim() ? `房主补充设定：
${room.custom_world.trim()}` : '';

  const historyText =
    history.length === 0
      ? '（故事刚刚开始，尚无历史。请直接以本回合玩家行动为起点展开。）'
      : history.map((s) => `第 ${s.round} 回合：
${s.narrative || s.world_events}`).join('
...
');
  ...
  const actionsText =
    actions.length === 0
      ...
      : actions.map((a) => `- ${a.player_name}：${a.content}`).join('
');

  return `【世界观】${worldName}：${worldDesc}
${worldExtra}

【故事历史】
${historyText}

【第 ${room.current_round} 回合 · 玩家行动】
${actionsText}

请扮演故事编辑，依据上述历史与行动推进故事，并严格按照系统提示输出 JSON。`;
}
...
export function buildOpeningUserMessage(room: RoomRow, playerNames: string[]): string {
  ...
  const worldName = world?.name ?? room.world_setting;
  const worldDesc = world?.description ?? '';
  const worldExtra = room.custom_world?.trim() ? `
房主补充设定：${room.custom_world.trim()}` : '';
  return `【世界观】${worldName}：${worldDesc}${worldExtra}
【参与玩家】${playerNames.join('、')}
请为这个世界撰写开场，只输出 JSON 对象 { "opening": "..." }。`;
}
...
function composeNarrative(worldEvents: string, outcomes: Record<string, string>): string {
  ...
  if (names.length) parts.push(names.map((n) => `${n}：${outcomes[n]}`).join('
'));
  return parts.join('
